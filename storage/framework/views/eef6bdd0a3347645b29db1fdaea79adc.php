                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <select wire:model="reservation_id" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            ...
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $pendingReservations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $res): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($res->id); ?>">Reserva #<?php echo e($res->id); ?> - <?php echo e($res->client->nombre ?? 'Sin cliente'); ?> (Total: $<?php echo e($res->precio_total ?? 0); ?>)</option>
                ...
                <tbody class="divide-y divide-slate-200">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $payments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $payment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="px-6 py-4">
                                <p class="font-medium text-slate-900"><?php echo e($payment->reservation->client->nombre ?? 'N/A'); ?></p>
                                <p class="text-xs text-slate-500">Reserva #<?php echo e($payment->reservation_id); ?></p>
                            </td>
                            <td class="px-6 py-4">
                                <span class="font-bold text-slate-900">$<?php echo e(number_format($payment->monto, 2)); ?></span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-2.5 py-1 bg-slate-100 text-slate-600 rounded text-xs font-medium border border-slate-200">
                                    <?php echo e(ucwords(str_replace('_', ' ', $payment->metodo_pago))); ?>

                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border
                                    <?php if($payment->estado === 'completado'): ?> bg-green-50 text-green-700 border-green-200
                                    <?php elseif($payment->estado === 'pendiente'): ?> bg-amber-50 text-amber-700 border-amber-200
                                    <?php elseif($payment->estado === 'fallido'): ?> bg-red-50 text-red-700 border-red-200
                                    <?php else: ?> bg-blue-50 text-blue-700 border-blue-200
                                    <?php endif; ?>">
                            ...
                            <td class="px-6 py-4 text-slate-600 text-sm">
                                <?php echo e($payment->fecha_pago ? \Carbon\Carbon::parse($payment->fecha_pago)->format('d M, Y H:i') : 'N/A'); ?>

                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-3">
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($payment->estado === 'pendiente'): ?>
                                        <button wire:click="markAsCompleted(<?php echo e($payment->id); ?>)" title="Marcar como completado" class="text-green-600 hover:text-green-800 transition">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                        </button>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                    <button wire:click="deletePayment(<?php echo e($payment->id); ?>)" onclick="return confirm('¿Eliminar permanentemente este registro de pago?')" title="Eliminar" class="text-red-500 hover:text-red-700 transition">
