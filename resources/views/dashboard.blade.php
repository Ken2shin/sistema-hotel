                    ...
                    <tbody class="divide-y divide-slate-200">
                        @forelse($recent_reservations ?? [] as $reservation)
                            <tr class="hover:bg-slate-50 transition">
                                <td class="px-6 py-3 text-slate-900">{{ $reservation->client->nombre ?? 'N/A' }}</td>
                                <td class="px-6 py-3 text-slate-900">{{ $reservation->room->numero ?? 'N/A' }}</td>
                                <td class="px-6 py-3 text-slate-600">{{ $reservation->fecha_inicio ? \Carbon\Carbon::parse($reservation->fecha_inicio)->format('d M, Y') : 'N/A' }}</td>
                                <td class="px-6 py-3">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold
                                        @if($reservation->estado === 'confirmada') bg-green-100 text-green-700
                    ...
                    <div class="bg-slate-50 rounded-lg p-4">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="w-full bg-slate-200 rounded-full h-2 mr-3">
                                <div class="bg-purple-600 h-2 rounded-full transition-all" style="width: {{ $occupancy_rate ?? 0 }}%"></div>
                            </div>
                            <span class="text-sm font-bold text-slate-900">{{ $occupancy_rate ?? 0 }}%</span>
                        </div>
