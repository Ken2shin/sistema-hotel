        ...
        <div class="bg-white/70 backdrop-blur-xl rounded-3xl border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-6 transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:-translate-y-1">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-4xl font-extrabold text-slate-800 mt-2">{{ $stats['total_reservations'] ?? 0 }}</p>
                </div>
        ...
        <div class="bg-white/70 backdrop-blur-xl rounded-3xl border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-6 transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:-translate-y-1">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-4xl font-extrabold text-slate-800 mt-2">{{ $stats['total_clients'] ?? 0 }}</p>
                </div>
        ...
        <div class="bg-white/70 backdrop-blur-xl rounded-3xl border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-6 transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:-translate-y-1">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-4xl font-extrabold text-slate-800 mt-2">${{ number_format($stats['total_revenue'] ?? 0, 2) }}</p>
                </div>
                    ...
                    <tbody class="divide-y divide-slate-100/60">
                        @forelse($recent_reservations ?? [] as $reservation)
                            <tr class="hover:bg-slate-50/50 transition duration-200">
                                <td class="px-7 py-4 text-slate-800 font-semibold">{{ $reservation->client->nombre ?? 'N/A' }}</td>
                                <td class="px-7 py-4 text-slate-600">{{ $reservation->room->numero ?? 'N/A' }}</td>
                                <td class="px-7 py-4 text-slate-600">{{ $reservation->fecha_inicio ? \Carbon\Carbon::parse($reservation->fecha_inicio)->format('d M, Y') : 'N/A' }}</td>
                                <td class="px-7 py-4">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold
                                        @if($reservation->estado === 'confirmada') bg-emerald-100 text-emerald-700
                                        @elseif($reservation->estado === 'pendiente') bg-amber-100 text-amber-700
                                        @else bg-red-100 text-red-700
                                        @endif">
                                        {{ match($reservation->estado) {
