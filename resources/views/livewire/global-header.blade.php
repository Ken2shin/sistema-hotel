<header class="bg-white/80 backdrop-blur-xl border-b border-slate-200 shadow-sm sticky top-0 z-40">
    <div class="flex items-center justify-between px-6 py-3">
        ...
        <div class="flex items-center space-x-4">
            ...
            <!-- BUSCADOR GLOBAL -->
            <div class="relative hidden sm:block group" x-data="{ open: @entangle('search').live.length > 0 }" @click.outside="open = false">
                    ...
                    <div x-show="open" x-transition x-cloak class="absolute top-full mt-3 w-[400px] right-0 sm:right-auto sm:left-0 bg-white rounded-3xl shadow-[0_10px_40px_rgb(0,0,0,0.1)] border border-slate-100 overflow-hidden z-50">
                        ...
                        <ul class="max-h-[60vh] overflow-y-auto">
                            @foreach($searchResults as $result)
                                <li>
                                    <a href="{{ $result['url'] }}" class="flex items-center px-4 py-3 hover:bg-slate-50 transition-colors group border-b border-slate-50 last:border-0">
                                        <div class="w-10 h-10 rounded-full {{ $result['icon_bg'] }} flex items-center justify-center {{ $result['icon_color'] }} mr-4 group-hover:scale-110 transition-transform">
                                            <i class="fa-solid {{ $result['icon'] }}"></i>
                                        </div>
                                        <div>
                                            <p class="text-sm font-bold text-slate-800">{{ $result['title'] }}</p>
                                            <p class="text-xs text-slate-500">{{ $result['type'] }} &bull; {{ $result['subtitle'] }}</p>
                                        </div>
                                    </a>
                                </li>
                            @endforeach
                ...
                @elseif(strlen($search) >= 2)
                    <div x-show="open" x-transition x-cloak class="absolute top-full mt-3 w-72 right-0 sm:right-auto sm:left-0 bg-white rounded-3xl shadow-[0_10px_40px_rgb(0,0,0,0.1)] border border-slate-100 p-4 text-center z-50">
                        <p class="text-sm text-slate-500 font-medium">No se encontraron resultados para "{{ $search }}"</p>
                    </div>
                @endif
            </div>
