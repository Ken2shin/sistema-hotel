...
<head>
    ...
    <title>@yield('title', 'HotelMS - Enterprise ERP')</title>
    ...
    <div class="flex h-screen bg-slate-50 overflow-hidden">
                ...
                <nav class="flex-1 overflow-y-auto px-3 py-6 space-y-1">
                    @auth
                        @if($canAccess('dashboard'))
                            <a href="{{ route('dashboard') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('dashboard') ? 'active' : '' }}">
                        ...
                        @if($canAccess('reservations'))
                            <a href="{{ route('admin.reservations') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.reservations') ? 'active' : '' }}">
                        ...
                        @if($canAccess('clients'))
                            <a href="{{ route('admin.clients') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.clients') ? 'active' : '' }}">
                        ...
                        @if($canAccess('rooms'))
                            <a href="{{ route('admin.rooms') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.rooms') ? 'active' : '' }}">
                                <i class="fa-solid fa-bed w-5 text-center"></i>
                                <span class="text-sm">Habitaciones</span>
                            </a>
                            
                            <a href="{{ route('admin.images') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg ml-2 {{ request()->routeIs('admin.images') ? 'active' : '' }}">
                                <i class="fa-regular fa-images w-5 text-center text-xs"></i>
                                <span class="text-sm">Galería de Imágenes</span>
                            </a>
                            <a href="{{ route('hotel.visualization') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg ml-2 {{ request()->routeIs('hotel.visualization') ? 'active' : '' }}">
                        ...
                        @if($canAccess('payments'))
                            <a href="{{ route('admin.payments') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.payments') ? 'active' : '' }}">
                        ...
                        @if($canAccess('reports'))
                            <a href="{{ route('admin.reports') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.reports') ? 'active' : '' }}">
                        ...
                        @if($canAccess('settings'))
                            <a href="{{ route('admin.settings') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.settings') ? 'active' : '' }}">
                        ...
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" class="w-full flex items-center space-x-3 px-3 py-2 rounded-lg hover:bg-slate-800 transition">
                                ...
                                <div class="flex-1 text-left overflow-hidden">
                                    <p class="text-sm font-bold text-slate-200 truncate">{{ auth()->user()->name ?? 'Usuario' }}</p>
                                    <p class="text-[11px] text-slate-400 font-mono truncate">{{ auth()->user()->role->name ?? 'Empleado' }}</p>
                                </div>
        ...
        <div class="flex-1 flex flex-col h-screen overflow-hidden">
            ...
            <header class="bg-white border-b border-slate-200 shadow-sm sticky top-0 z-40">
                <div class="flex items-center justify-between px-6 py-3">
                    <div class="flex items-center gap-4">
                        ...
                        <span class="hidden md:block text-slate-400 text-sm font-medium"><i class="fa-regular fa-clock mr-1"></i> {{ date('d M Y') }}</span>
                    </div>
            ...
            <footer class="bg-white border-t border-slate-200 py-3 px-6 text-center text-xs text-slate-500 font-medium">
                <p>&copy; {{ date('Y') }} Hotel Management System ERP. Todos los derechos reservados.</p>
            </footer>
        </div>
    </div>
